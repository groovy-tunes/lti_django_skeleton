...
class Course(models.Model):
    ...
    external_id = CharField(max_length=255, default="")
...
class Submission(models.Model):
    ...
    user= ForeignKey(LTIUser, on_delete=models.CASCADE)
    assignment_version = IntegerField(default=0)
...
class Assignment(Base):
    ...
    on_run = TextField(default="def on_run(code, output, properties):\n    return True")
    on_step = TextField(default="def on_step(code, output, properties):\n    return True")
    on_start = TextField(default="")
    answer = TextField(default="")
    due = DateTimeField(default=None)
    type = CharField(max_length=10, default="normal")
    visibility = CharField(max_lenght=10, default="visible")
    disabled = CharField(max_length=10, default="enabled")
    mode = CharField(max_length=10, default="blocks")
    owner = ForeignKey(LTIUser, on_delete=models.CASCADE)
    course = ForeignKey(Course, on_delete=models.CASCADE)
    version = IntegerField(default=0)
...
class AssignmentGroupMembership(Base):
    assignment_group = ForeignKey(AssignmentGroup, on_delete=models.CASCADE)
    assignment = ForeignKey(Assignement, on_delete=models.CASCADE)
...
class Log(Base):
    event = CharField(max_length=255, default="")
